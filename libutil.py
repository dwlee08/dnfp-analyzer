...
#계시 : 아리아, 퍼페티어 지능 173 증가
    ...
    b_parser = [
            ...
            compile("50Lv액티브스킬힘지능증가량{포힘지:g}%증가"), 
    ...
    @classmethod
    def parse_explain(cls, explain, io, name, skill_db, step = 0):
        ...
        if name.find('사도 강림 플래티넘') >= 0 or name.find('위대한 의지') >= 0 or name.find('강인한 사도') >= 0:
            p = compile("{}[{lv:d}Lv]")
            result = p.parse(name)
            if result is not None:
                lv = result['lv']
            ...
            io.append({'스킬':({'job':'도적','jid':cls.get_jobid('도적', skill_db)},
                        [{'minLevel':1, 'maxLevel':45, 'value':3},
        ...
        elif name == '지체없는 흐름의 한뉘' or name == '영명한 세상의 순환':
             io.append({'스킬':({'job':'공통', 'jid':None},
                        [
        ...
        elif name == '새벽을 녹이는 따스함' or name == '새벽을 감싸는 따스함':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
        ...
        elif name == '시간에 휩쓸린 물소 각반':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
                            {'minLevel':60, 'maxLevel':80, 'cooldown':10},
                        ]
                      )})
        elif name == '시간을 거스르는 자침' or name == '시간을 가리키는 지침':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
        ...
        elif name == '차원을 걷는 물소 부츠':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
                            {'minLevel':1, 'maxLevel':45, 'value':1},
                        ]
                      )})
        elif name == '차원을 지나는 자의 인장':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
                            {'minLevel':60, 'maxLevel':80, 'value':1},
                        ]
                      )})
        elif name == '차원을 관통하는 초신성' or name == '차원을 맴도는 혜성':
            io.append({'스킬':({'job':'공통', 'jid':None},
                        [
                            {'minLevel':50, 'maxLevel':50, 'value':1},
                            {'minLevel':85, 'maxLevel':85, 'value':1},
        ...
        elif name == '무너진 세상의 슬픔' or name == '광란을 품은 자의 종막' or name == '슬픔을 담은 운명':
            for opts in io:
                if '추댐' in opts.keys():
            ...
            for opts in io:
                if '스공' in opts.keys() and opts['스공'] == 4:
                    opts['스공'] = 1
        elif name in ['전쟁의 시작', '오퍼레이션 델타', '퀘이크 프론', '데파르망', '전장의 매']:
            io.pop(0)
    ...
    """
    @classmethod
    def build_single_item(cls, ids, skill_db, item_db):
        item_ids = ','.join(ids)
        ...
        for cur in item_dict['rows']:
            ...
            cls.parse_explain(explain, item['options'], name, skill_db, step = -1)
                ...
                if skills is not None and len(skills) > 0:
                    for skill in skills[0]['skills']:
                        if skill['name'] in ['마리오네트', '아포칼립스', '크럭스 오브 빅토리아']:
                            item['buffopts'].append({'포레벨':skill['value']})
                        elif skill['name'] in ['영광의 축복', '용맹의 축복', '금단의 저주']:
                            item['buffopts'].append({'축레벨':skill['value']})
                        elif skill['name'] in ['소악마', '신실한 열정', '신념의 오라']:
                            item['buffopts'].append({'오라레벨':skill['value']})
                        else:
                            print(skills)
                            raise Exception
    ...
    @classmethod
    def build_set_item(cls, name, skill_db, item_db, set_db):
        ...
        try:
            for slist in s_search['rows']:
                ...
                cls.build_single_item(ids, skill_db, item_db)
